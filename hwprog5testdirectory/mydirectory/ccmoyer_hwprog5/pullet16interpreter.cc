 ...
 *   3. Converts the ASCII to binary and writes binary to file
 *   4. Reads back the binary from the file and expands the binary
 *      into a new vector of ASCII strings
 *   5. Checks that the new vector is identical to the old vector
 *
...
void Interpreter::HW5Binary(string adotout_filename, ofstream& out_stream) {
  ...
  ascii_scanner.OpenFile(adotout_filename + ".txt");
  ReadProgram(ascii_scanner);
  ascii_scanner.Close();
  out_stream << "Dumping ASCII Vector" << endl;
  for (int i = 0; i < memory_.size(); ++i) {
    out_stream << memory_.at(i).GetBitPattern() << endl;
  }

  // Uses a bitset to convert the ascii to binary and then writes binary to
  // a file 16 bits at a time
  ofstream output_file(adotout_filename + ".bin", ofstream::binary);
  if (output_file) {
  ...
  ifstream is(adotout_filename + ".bin", ifstream::binary);
  if (is) {
    is.seekg(0, is.end);
    int length = is.tellg();
    is.seekg(0, is.beg);
    ...
    for (int i = 0; i < length; i+=2) {
      int16_t the_binary;
      is.read(reinterpret_cast<char*>(&the_binary), sizeof(int16_t));
      memory_from_bin_.push_back(DABnamespace::DecToBitString(the_binary, 16));
    }
  is.close();

    // Prints out the ASCII that was read back from the binary file
    out_stream << "Printing reconstructed vector" << endl;
    for (int i = 0; i < memory_from_bin_.size(); ++i) {
      out_stream << memory_from_bin_.at(i) << endl;
    }
  }

  // Check that the original ASCII vector and read from binary file
  // vector are equal
  bool vectors_are_equal = true;

    for (int i = 0; i < memory_.size(); ++i) {
       if (memory_.at(i).GetBitPattern() != memory_from_bin_.at(i)) {
         vectors_are_equal = false;
         break;
       }
    }

  if (vectors_are_equal == true) {
    out_stream << "Vectors are equal" << endl;
  } else {
    out_stream << "Vectors are not equal" << endl;
  }
